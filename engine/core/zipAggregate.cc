...
ZipAggregate::createZipDirectory(Stream* io_pStream)
{
	...
	if (streamSize < sizeof(ZipEOCDRecord::EOCDRecord)) return false;
	...
	// Read comment (if any)
	io_pStream->setPosition(streamSize - 3);

	U8 commentTest = 0;
	io_pStream->read(&commentTest);
	if (commentTest != 0) {
		AssertWarn(false, "Zip file has a comment; Searching for ECOD...");

		bool foundStart     = false;
		S32 currentPosition = S32(io_pStream->getPosition() - sizeof(U32));
		for (S32 i = 0; (currentPosition - i) >= 0; i++) {
			io_pStream->setPosition(U32(currentPosition - i));

			U8 byte = 0;
			bool success = io_pStream->read(&byte);
			if (success && byte == 0) {
				// Check if the position is valid
				if ((currentPosition - (i + sizeof(ZipEOCDRecord::EOCDRecord) - 1)) < 0) break;

				io_pStream->setPosition(U32(currentPosition - (i + sizeof(ZipEOCDRecord::EOCDRecord) - 1)));
				foundStart = true;
				break;
			}
		}
		...
		if (!foundStart) {
			AssertWarn(false, "Zip file is invalid");
			return false;
		}
	} else io_pStream->setPosition(streamSize - sizeof(ZipEOCDRecord::EOCDRecord));
