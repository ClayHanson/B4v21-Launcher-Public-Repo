...
ResourceInstance * ResManager::loadInstance (ResourceObject * obj, bool computeCRC)
{
   ...
   bool stop = false;
   DO_CREATE_FUNCTION:
   RESOURCE_CREATE_FN createFunction = ResourceManager->getCreateFunction (name);

   if(!createFunction)
   {
       AssertWarn( false, "ResourceObject::construct: NULL resource create function.");
       Con::errorf("ResourceObject::construct: NULL resource create function for '%s'.", name);
       return NULL;
   }
   ...
   if (!ret && !stop) {
	   ...
	   // JPEG = "\xFF\xD8\xFF\xEE"
	   // JPEG = "\xFF\xD8\xFF\xE1"
	   // PNG  = "\x89\x50\x4E\x47\x0D\x0A\x1A\x0A"
	   // BMP  = "\x42\x4D"
	   if (newStream.open(avar("%s/%s", obj->path, obj->name), FileStream::AccessMode::Read)) {
		   char buffer[17];
		   newStream.read(16, buffer);
		   newStream.close();

		   const char* s = "";
		   if (dStrstr((const char*)buffer, "\xFF\xD8\xFF\xDB") == buffer) s = ".jpg";
		   if (dStrstr((const char*)buffer, "\xFF\xD8\xFF\xE0") == buffer) s = ".jpg";
		   if (dStrstr((const char*)buffer, "\xFF\xD8\xFF\xEE") == buffer) s = ".jpg";
		   if (dStrstr((const char*)buffer, "\xFF\xD8\xFF\xE1") == buffer) s = ".jpg";
		   if (dStrstr((const char*)buffer, "\x89\x50\x4E\x47\x0D\x0A\x1A\x0A") == buffer) s = ".png";
		   if (dStrstr((const char*)buffer, "\x42\x4D") == buffer) s = ".bmp";

		   if (dStrstr(obj->name, s) != NULL) return NULL;

		   char newFileName[1024];
		   dSprintf(newFileName, 1024, "%s", obj->name);
		   *(newFileName + ((newFileName + dStrlen(newFileName)) - dStrrchr(newFileName, '.'))) = 0;
		   dStrcat(newFileName, s);

		   name = StringTable->insert(newFileName);
		   stop = true;

		   (*stream).setPosition(0);
		   
		   goto DO_CREATE_FUNCTION;
	   } else return NULL;
   }
...
ResourceObject* ResManager::find(const char* fileName)
{
	...
	if (!ret)
	{
		...
		// Search for this object in a zip file
		StringTableEntry tmpPath, tmpFile;
		bool found = false;
		tmpPath    = (path ? path : file);
		char zipFile[512];
		dSprintf(zipFile, sizeof(zipFile), "%s.zip", tmpPath);
		while (!(found = Platform::isFile(zipFile)))
		{
			StringTableEntry nPath;
			getPaths(tmpPath, nPath, tmpFile);
			tmpPath = nPath;
			dSprintf(zipFile, sizeof(zipFile), "%s.zip", tmpPath);
			if (!tmpPath)
				break;
		...
		{
			ZipAggregate zipAggregate;
			if (zipAggregate.openAggregate(zipFile) == false)
			{
				Con::errorf("Error opening zip (%s), need to handle this better...", zipFile);
				return ret;
			}

			getPaths(zipFile, tmpPath, tmpFile);

			// Create resource objects for every file
			for (ZipAggregate::iterator itr = zipAggregate.begin(); itr != zipAggregate.end(); itr++)
			{
				const ZipAggregate::FileEntry& rEntry = *itr;

				ResourceObject* newObj = createResource(rEntry.pPath, rEntry.pFileName);
				dictionary.pushBehind(newObj, ResourceObject::VolumeBlock);

				newObj->flags      = ResourceObject::Flags::VolumeBlock;
				newObj->fileOffset = rEntry.fileOffset;

				newObj->zipName            = StringTable->insert(tmpFile);
				newObj->zipPath            = StringTable->insert(tmpPath);
				newObj->fileSize           = rEntry.fileSize;
				newObj->compressedFileSize = rEntry.compressedFileSize;

				if (!dStricmp(rEntry.pPath, path) && !dStricmp(rEntry.pFileName, file))
				{
					// Found it! Set the ret value appropriately
					ret = newObj;
				}
			}
...
bool ResManager::openFileForWrite (FileStream & stream, const char *fileName, U32 accessMode, bool alwaysWritable)
{
   if (!alwaysWritable && !isValidWriteFileName (fileName))
      return false;
