...
bool FileObject::openForWrite(const char *fileName, const bool append)
{
	...
	if (dStrlen(fileName) >= 3 && *(fileName + 1) == ':' && (*(fileName + 2) == '/' || *(fileName + 2) == '\\')) dStrcpy(buffer, fileName);
	else Con::expandScriptFilename(buffer, sizeof(buffer), fileName);

	close();

	if (!append)
		return(ResourceManager->openFileForWrite(stream, buffer, 1U, true));

	// Use the WriteAppend flag so it doesn't clobber the existing file:
	if (!ResourceManager->openFileForWrite(stream, buffer, File::WriteAppend, true))
		return(false);
