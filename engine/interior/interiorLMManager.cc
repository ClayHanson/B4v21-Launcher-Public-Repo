...
U32 InteriorLMManager::getLightMapMemoryUsage()
{
   ...
   {
      InteriorLMInfo *interiorInfo = mInteriors[i];
      for(U32 j=0; j<interiorInfo->mInstances.size(); j++)
      {
         InstanceLMInfo * instanceInfo = interiorInfo->mInstances[j];
         for(U32 k=0; k<instanceInfo->mLightmapHandles.size(); k++)
         {
            TextureHandle *texh = instanceInfo->mLightmapHandles[k];
            if(!texh)
               continue;

            mem += texh->getHeight() * texh->getWidth() * 3;
         }
      }
   }
