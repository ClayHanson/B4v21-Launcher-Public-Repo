...
ConsoleFunction(atoi, S32, 2, 2, "(string)"
	"Convert a string to a number")
{
	return dAtoi(argv[1]);
}

ConsoleFunction(atof, F32, 2, 2, "(string)"
	"Convert a string to a floating point number")
{
	return dAtof(argv[1]);
}

ConsoleFunction(mClamp, S32, 4, 4, "(string, min, max)"
	"Clamp a value between two values")
{
	S32 value = dAtoi(argv[1]);
	S32 min = dAtoi(argv[2]);
	S32 max = dAtoi(argv[3]);

	return (value > max ? max : (value < min ? min : value));
}

ConsoleFunction(mClampF, F32, 4, 4, "(string, min, max)"
	"Clamp a value between two values")
{
