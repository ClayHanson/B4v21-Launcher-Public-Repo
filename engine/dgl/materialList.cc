...
void MaterialList::load(U32 index, const char* path)
{
	...
	if (index < size())
	{
		...
		if (name && *name)
		{
			const char* end = path + dStrlen(path);
			char buffer[512];
			while (true)
			{
				// Build the string
				*buffer = 0;
				dStrncat(buffer, path, end - path);
				dStrcat(buffer, "/");
				dStrcat(buffer, name);


				// Attempt to load it
				if (handle.set(buffer, mTextureType, mClampToEdge))
					break;

				// Find the next end point
				*buffer = 0;
				dStrncat(buffer, path, (end - 1) - path);
				if ((end = (dStrrchr(buffer, '/'))) == NULL && (end = (dStrrchr(buffer, '\\'))) == NULL)
					break;
