...
#include "game/net/ThreadedDownloading.h"
...
GLenum sgCompressionHint      = GL_NICEST;
...
TextureObject* TextureManager::registerTexture(const char* textureName, GBitmap* bmp, TextureHandleType type, bool clampToEdge)
{
   ...
   {
      // Crucial conditionals for the flush case...
      if (ret->bitmap != bmp && ret->type != BitmapKeepTexture)
         delete ret->bitmap;
      if (ret->texGLName)
         glDeleteTextures(1, (const GLuint*)&ret->texGLName);
      if (ret->smallTexGLName)
...
void TEXMGR_DOWNLOADER_onProgress(void* userData, S32 curr, S32 max)
{
	TextureObject* tex = (TextureObject*)userData;
	tex->DL_progress   = S32(F32((F32)curr / (F32)max) * 100);
}

void TEXMGR_DOWNLOADER_onComplete(void* userData, const char* file, S32 response)
{
	TextureObject* tex = (TextureObject*)userData;
	tex->downloading   = false;
	tex->DL_handle     = -1;
	tex->DL_progress   = 0;

	if (response == 200)
	{
		GBitmap *bmp = TextureManager::loadBitmapInstance(file);
		if (bmp == NULL)
			return;

		TextureManager::registerTexture(tex->texFileName, bmp, tex->type, tex->clamp);
	}
	else
	{
		Con::errorf("Failed to download %s.", file);
	}
...
TextureObject *TextureManager::loadTexture(const char* textureName, TextureHandleType type, bool clampToEdge, bool checkOnly /* = false */)
{
	...
	if (!bmp) {
		// Okay, that didn't work either. Let's try a URLInfo finder...
		UrlStructInfo urlInfo;
		if (urlInfo.setURL(textureName)) {
			TextureObject* toRet;

			char fileName[1024];
			urlInfo.getPathFileName(fileName);

			char fileExt[256];
			if (!urlInfo.getPathFileExt(fileExt) || (dStricmp(fileExt, ".png") && dStricmp(fileExt, ".jpeg") && dStricmp(fileExt, ".jpg") && dStricmp(fileExt, ".bmp")))
			{
				if (!*fileExt)
				{
					dStrcpy(fileExt, ".jpg");
					dStrcat(fileName, ".jpg");
				}
				else
				{
					Con::errorf("Invalid file extension \"%s\"; Trying anyways", fileExt);
					dStrcpy(fileExt, ".jpg");
					dStrcat(fileName, ".jpg");
				}
			}

			const char* savePath = avar("launcher/tmp/%s", fileName);

			if (Platform::isFile(savePath))
			{
				bmp = loadBitmapInstance(savePath, false);

				if (bmp)
					return registerTexture(savePath, bmp, type, clampToEdge);
				//if ((toRet = registerTexture(savePath, loadBitmapInstance(savePath), BitmapTexture, true)) != NULL)
					//return toRet;

				// File didn't load correctly; re-download it
			}

			// First set a temporary texObj
			if ((toRet = registerTexture(savePath, loadBitmapInstance("launcher/ui/img_invalid.png"), BitmapTexture, true)) == NULL)
			{
				Con::errorf("Failed to find img_invalid.png");
				return NULL;
			}

			if ((toRet->DL_handle = ThreadedDownloading::Download(textureName, savePath)) == ThreadedDownloading::INVALID_DOWNLOAD_ID)
			{
				Con::printf("Download system # 1 failed to init download; trying the other...");

				if ((toRet->DL_handle = downloadTCPFile(urlInfo.getHostName(), urlInfo.getPort(), urlInfo.getPath(), savePath, "\r\n", NULL, NULL, NULL, NULL, TEXMGR_DOWNLOADER_onComplete, TEXMGR_DOWNLOADER_onProgress, toRet)) == -1)
				{
					Con::errorf("Failed to initiate a download with host name \"%s\", port \"%s\", and path \"%s\".", urlInfo.getHostName(), urlInfo.getPort(), urlInfo.getPath());
					return toRet;
				}
			}
			else
			{
				ThreadedDownloading::QueuedDownload* pEntry = ThreadedDownloading::GetQueueEntry(toRet->DL_handle);
				...
				pEntry->onDownloadComplete.AddListener([](void* userData, U32 argc, char* argList)->void
				{
					...
					args[0] = userData;
					args[1] = dStrdup(pFileName);

					// Execute on main thread
					Sim::postEvent(Sim::getRootGroup(), 
						new SimEngineEvent(
							[](void* udata)->void
							{
								void** args = (void**)udata;
								TEXMGR_DOWNLOADER_onComplete(args[0], (const char*)args[1], 200);
								dFree(args[1]);
								delete[] args;
							},
							args
						),
					Sim::getCurrentTime() + 32);
				}, toRet);

				pEntry->onDownloadFailed.AddListener([](void* userData, U32 argc, char* argList)->void
				{
					TEXMGR_DOWNLOADER_onComplete(userData, NULL, 404);
				}, toRet);
				...
				pEntry->onDownloadProgress.AddListener([](void* userData, U32 argc, char* argList)->void
				{
					...
					TEXMGR_DOWNLOADER_onProgress(userData, iContentLength, iContentLengthMax);
				}, toRet);
			}

			toRet->downloading = true;
			return toRet;
		}
