...
//#define USE_NEW_PNG_READER 1

#ifdef USE_NEW_PNG_READER
#include "dgl/lodepng/lodepng.h"

bool GBitmap::readPNG(Stream& io_rStream)
{
	static U8* sBitmapBuffer     = NULL;
	static S32 sBitmapBufferSize = 0;
	...
	if ((size = io_rStream.getStreamSize()) > sBitmapBufferSize)
	{
		if (sBitmapBuffer)
		{
			// Resize it
			sBitmapBuffer     = (U8*)dRealloc((void*)sBitmapBuffer, size);
			sBitmapBufferSize = size;
		}
		else
		{
			// Allocate a new one
			sBitmapBuffer     = (U8*)dMalloc(size);
			sBitmapBufferSize = size;
		}
	}
	...
	io_rStream.read(size, sBitmapBuffer);

	// Attempt to decode the png
	if ((error = lodepng_decode(&pixelData, &width, &height, &s, sBitmapBuffer, size)) != 0)
	{
		AssertWarn(false, avar("GBitmap::readPNG() - Failed to read PNG: \"%s\"", lodepng_error_text(error)));
		return false;
	}

	// Determine the format
	switch (s.info_raw.colortype)
	{
	...
	U32 bpp = lodepng_get_bpp(&s.info_raw) >> 3;

	// Set up the row pointers...
	dMemcpy(getAddress(0, 0), pixelData, (width * height) * bpp);
...
bool GBitmap::readPNG(Stream& io_rStream)
{
   ...
   {
	   AssertWarn(false, "Error, cannot load pngs taller than 2048 pixels!");
	   return false;
   }
