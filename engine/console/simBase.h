...
/// Implementation of calling engine functions
///
/// This allows you to set an engine function to be
/// called at some point in the future.
class SimEngineEvent : public SimEvent
{
public:
	typedef void(*SimEngineEventFunc)(void* userData);

protected:
	SimEngineEventFunc mFuncPtr;
	void* mUserData;
...
public:
	...
	/// @see Con::execute(S32 argc, const char *argv[])
	/// @see Con::execute(SimObject *object, S32 argc, const char *argv[])
	SimEngineEvent(SimEngineEventFunc funcPtr, void* userData);
	~SimEngineEvent();
...
class SimObject: public ConsoleObject
{
  ...
  public:
   ...
   enum WriteFlags {
	  ...
	  WhitespaceFields = BIT(1) ///< Passed to SimObject::write to indicate that the fields written should have same-width spaces for fields.
   };
...
protected:
  ...
  public:
   ...
   virtual bool		save(const char* pcFilePath, bool bOnlySelected=false, S32 iInitialTab=0, bool bPretty=false);
   ...
   virtual void writeFields(Stream &stream, U32 tabStop, U32 flags = 0);
