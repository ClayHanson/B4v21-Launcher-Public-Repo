...
static void _printf(ConsoleLogEntry::Level level, ConsoleLogEntry::Type type, const char* fmt, va_list argptr)
{
   ...
   _vsprintf_p(buffer + offset, sizeof(buffer) - offset, fmt, argptr);
...
//------------------------------------------------------------------------------
bool isMethod(const char *cn, const char *fn) {
	const char *cnString = StringTable->lookup(cn);
	const char *fnString = StringTable->lookup(fn);
	if (!cnString || !fnString)
		return false;
	else {
		Namespace* ns = Namespace::find(cnString);

		if (!ns) return false;
		return ns->lookup(fnString) != NULL;
	}
}
