...
StringTableEntry gMatchMakerToken = NULL;
NetAddress gMatchMakerAddr;
S32 gMatchMakerSecret = 0;
...
void NetInterface::processPacketReceiveEvent(PacketReceiveEvent *prEvent)
{
   ...
   else
   {
      ...
      if(packetType <= SimplePingResponse)
      ...
      else
      {
         ...
         switch(packetType)
         {
			case ArrangedConnectionNotify:
				handleArrangedConnectionNotify(addr, &pStream);
				break;
			case PunchPacket:
				handlePunchPacket(addr, &pStream);
				break;
...
void NetInterface::handleArrangedConnectionNotify(const NetAddress* address, BitStream* stream)
{
	if (gMatchMakerAddr.type == NetAddress::IPAddress
		&& gMatchMakerAddr.netNum[0] == address->netNum[0]
		&& gMatchMakerAddr.netNum[1] == address->netNum[1]
		&& gMatchMakerAddr.netNum[2] == address->netNum[2]
		&& gMatchMakerAddr.netNum[3] == address->netNum[3])
	{
		S32 MM_Secret = 0;
		stream->read(4, &MM_Secret);
		if (Sim::findObject("ServerGroup") && MM_Secret != gMatchMakerSecret)
			Con::errorf("ERROR: match maker secret - expected %d, got %d", gMatchMakerSecret, MM_Secret);
	}

	Con::printf("got arranged connection notification!");
}

void NetInterface::handlePunchPacket(const NetAddress* address, BitStream* stream)
{
