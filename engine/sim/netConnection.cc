...
static U32 gPacketRateToServer = 256;
static U32 gPacketUpdateDelayToServer = 32;
static U32 gPacketRateToClient = 256;
static U32 gPacketSize = 1023;
...
void NetConnection::checkMaxRate()
{
   ...
   if(gPacketRateToServer > 256)
      gPacketRateToServer = 256;
   if(gPacketRateToServer < 8)
      gPacketRateToServer = 8;

   // Limit packet rate to client.
   if(gPacketRateToClient > 256)
      gPacketRateToClient = 256;
   if(gPacketRateToClient < 1)
      gPacketRateToClient = 1;

   // Limit packet size.
   if(gPacketSize > 1023) // 450 previously for some god forsaken reason
      gPacketSize = 1023;
   if(gPacketSize < 100)
      gPacketSize = 100;
