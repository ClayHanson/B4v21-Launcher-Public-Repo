...
void GuiTreeViewCtrl::removeSelection(S32 itemId)
{
   ...
   Con::executef(this, 2, "onRemoveSelection", buf);
}
...
void GuiTreeViewCtrl::addSelection(S32 itemId)
{
   ...
   if(!item)
   {
      ...
      for (S32 i = 0; i <mItems.size(); i++)
      {
         if (mItems[i] != 0)
         {
            if (mItems[i]->isInspectorData())
            {
               if ( mItems[i]->getObject() && mItems[i]->getObject()->getId() == itemId)
               {
                  item = mItems[i];
   ...
   char buf[16];
   dSprintf(buf, 16, "%d", item->mId);
   Con::executef(this, 2, "onAddSelection", buf);
}
