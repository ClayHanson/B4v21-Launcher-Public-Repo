...
{
#ifdef TORQUE_DEBUG
	bool DrawDebug = Con::getBoolVariable("DrawListIDs", false);
	if (DrawDebug)
	{
		char IDStr[128];
		dSprintf(IDStr, 128, "%d", mList[cell.y].id);
		RectI dbgSize(offset.x, offset.y, mFont->getStrWidthPrecise((char*)IDStr), mFont->getHeight());
		dglDrawRectFill(dbgSize, ColorI(0, 0, 0, 255));
		dglSetBitmapModulation(ColorI(255, 255, 255, 255));
		dglDrawTextN(mFont, offset, IDStr, dStrlen(IDStr));
		dglClearBitmapModulation();
		offset.x += dbgSize.extent.x;
	}
#endif
...
bool GuiTextListCtrl::onKeyDown( const GuiEvent &event )
{
   ...
   case KEY_UP:
      if ( mSelectedCell.y > 0 )
      {
		 ...
		 Con::executef(this, 3, "onSelect", Con::getIntArg(mList[mSelectedCell.y].id), mList[mSelectedCell.y].text);
   }
      break;
   case KEY_DOWN:
   case KEY_RIGHT:
