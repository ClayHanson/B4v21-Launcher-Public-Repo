...
void GuiTabPageCtrl::initPersistFields()
{
   ...
   addField("tabVisible", TypeBool, Offset(mTabVisible, GuiTabPageCtrl));
}
...
ConsoleMethod(GuiTabPageCtrl, setTabVisible, void, 3, 3, "(bool val)")
{
	object->setTabVisible(dAtob(argv[2]));
}

ConsoleMethod(GuiTabPageCtrl, getTabVisible, bool, 2, 2, "()")
{
	return object->getTabVisible();
}