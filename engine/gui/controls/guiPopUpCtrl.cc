...
void GuiPopUpMenuCtrl::initPersistFields(void)
{
   ...
   addField("useNewDesign",             TypeBool,         Offset(mNewDesign, GuiPopUpMenuCtrl));
}
...
void GuiPopUpMenuCtrl::onRender(Point2I offset, const RectI& updateRect)
{
	...
	if (mNewDesign)
	{
		...
		{
			dglDrawRect(r, ColorI(255, 255, 255, 255));
			dglDrawRect(RectI(r.point.x + 1, r.point.y + 1, r.extent.x - 1, r.extent.y - 1), mProfile->mBorderColor);
			dglDrawRectFill(RectI(r.point.x + 1, r.point.y + 1, r.extent.x - 2, r.extent.y - 2), mProfile->mFillColor);
		}
	}
	else
	{
...
void GuiPopUpMenuCtrl::onAction()
{
   ...
   mSc->mBounds.point = mBackground->globalToLocalCoord(scrollPoint) - Point2I(0, 1);
...
bool GuiPopUpMenuCtrl::getFontColor( ColorI &fontColor, S32 id, bool selected, bool mouseOver )
{
   ...
   fontColor = selected ? mProfile->mFontColorSEL : mouseOver ? (mNewDesign ? ColorI(255, 255, 255, 255) : mProfile->mFontColorHL) : mProfile->mFontColor;
