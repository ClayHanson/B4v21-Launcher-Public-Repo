...
{
	// Fill titlebar first.
	dglDrawRectFill(RectI(offset.x, offset.y, mBounds.extent.x, mThumbSize.y), mProfile->mFillColor);
	...
	if (mProfile->mBitmapArrayRects.size() >= 2 && mCollapsable)
	{
		...
		if (mProfile->mBitmapArrayRects[idx].extent.y < mProfile->mFont->getHeight()) pos.y += (mProfile->mFont->getHeight() / 2) - (mProfile->mBitmapArrayRects[idx].extent.y / 2);

		dglClearBitmapModulation();
		dglDrawBitmapStretchSR(
			mProfile->mTextureHandle,
			RectI(pos, mProfile->mBitmapArrayRects[idx].extent),
	...
	// Draw seperator
	dglDrawLine(Point2I(offset.x, offset.y + mThumbSize.y), Point2I(offset.x + mBounds.extent.x, offset.y + mThumbSize.y), mProfile->mBorderColor);
...
ConsoleMethod(GuiPaneControl, isCollapsed, bool, 2, 2, "")
{
	return object->getCollapsed();
}
