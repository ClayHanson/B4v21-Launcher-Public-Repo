...
void GuiTabBookCtrl::renderTabs(const Point2I& offset)
{
	...
	S32 invisibleOffset = 0;
	for (S32 i = 0; i < mPages.size(); i++)
	{
		RectI tabBounds = mPages[i].TabRect;
		tabBounds.point += offset;

		if (!mPages[i].Page->mTabVisible)
		{
			invisibleOffset += ((mTabPosition == TabPosition::AlignBottom || mTabPosition == TabPosition::AlignTop) ? tabBounds.extent.x : tabBounds.extent.y);
			continue;
		}

		if (invisibleOffset != 0)
		{
			if (mTabPosition == TabPosition::AlignBottom || mTabPosition == TabPosition::AlignTop)
				tabBounds.point.x -= invisibleOffset;
			else
				tabBounds.point.y -= invisibleOffset;
		}
...
GuiTabPageCtrl *GuiTabBookCtrl::findHitTab( Point2I hitPoint )
{
   ...
   for( S32 i = 0; i < mPages.size(); i++ )
   {
      ...
      if(tabBounds.pointInRect( hitPoint ) )
...
void GuiTabBookCtrl::selectPage( S32 index, bool user )
{
   if( mPages.size() < index )
      return;

   // Select the page
   selectPage( mPages[ index ].Page, user );
}


void GuiTabBookCtrl::selectPage( GuiTabPageCtrl *page, bool user )
{
   ...
   for( ; i != mPages.end() ; i++ )
   {
      ...
      if( page == tab )
      {
		 ...
		 if (user && tab->getConsoleCommand() && *tab->getConsoleCommand())
			 Con::evaluate(tab->getConsoleCommand());
