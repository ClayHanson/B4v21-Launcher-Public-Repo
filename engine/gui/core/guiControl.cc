...
static bool SetVisibleFunc(void* obj, const char* data)
{
	static_cast<GuiControl*>(obj)->setVisible(dAtob(data));
	return false;
}


static bool SetPositionFunc(void* obj, const char* data)
{
	Point2I newPosition;

	// Scan it
	if (dSscanf(data, "%d %d", &newPosition.x, &newPosition.y) != 2)
	{
		Con::errorf("SetPositionFunc() - Invalid position provided!");
		return false;
	}

	static_cast<GuiControl*>(obj)->setPosition(newPosition);
	return false;
}


static bool SetExtentFunc(void* obj, const char* data)
{
	Point2I newExtent;

	// Scan it
	if (dSscanf(data, "%d %d", &newExtent.x, &newExtent.y) != 2)
	{
		Con::errorf("SetExtentFunc() - Invalid extent provided!");
		return false;
	}

	static_cast<GuiControl*>(obj)->setExtent(newExtent);
	return false;
}
...
void GuiControl::initPersistFields()
{
   ...
   addProtectedField("Position",          TypePoint2I,      Offset(mBounds.point, GuiControl), &SetPositionFunc, &defaultProtectedGetFn, "");
   addProtectedField("Extent",            TypePoint2I,      Offset(mBounds.extent, GuiControl), &SetExtentFunc, &defaultProtectedGetFn, "");
   addField("MinExtent",         TypePoint2I,      Offset(mMinExtent, GuiControl));
   addField("canSave",           TypeBool,         Offset(mCanSave, GuiControl));
   addProtectedField("Visible",  TypeBool,         Offset(mVisible, GuiControl), &SetVisibleFunc, &defaultProtectedGetFn, "");
...
void GuiControl::awaken()
{
   ...
   if(!mAwake)
   {
	  ...
	  if (!OnVisible())
	  {
		  Con::errorf(ConsoleLogEntry::General, "GuiControl::awaken: failed onVisible for obj: %s", getName());
		  AssertFatal(0, "GuiControl::awaken: failed onVisible");
		  deleteObject();
		  return;
	  }
   }
...
bool GuiControl::OnVisible()
{
	return true;
}

void GuiControl::OnInvisible()
{
}
...
ConsoleMethod( GuiControl, getLeft, S32, 2, 2, "Get the X position of the control.")
{
	return object->getLeft();
}

ConsoleMethod( GuiControl, getTop, S32, 2, 2, "Get the Y position of the control.")
{
	return object->getTop();
}

ConsoleMethod( GuiControl, getWidth, S32, 2, 2, "Get the width of the control.")
{
	return object->getWidth();
}

ConsoleMethod( GuiControl, getHeight, S32, 2, 2, "Get the height of the control.")
{
...
ConsoleMethod( GuiControl, setLeft, void, 3, 3, " sets the X position of the control.")
{
   object->setLeft(dAtoi(argv[2]));
}

ConsoleMethod( GuiControl, setTop, void, 3, 3, " sets the Y position of the control.")
{
   object->setTop(dAtoi(argv[2]));
}

ConsoleMethod( GuiControl, setWidth, void, 3, 3, " sets the width of the control.")
{
   object->setWidth(dAtoi(argv[2]));
}

ConsoleMethod( GuiControl, setHeight, void, 3, 3, " sets the height of the control.")
{
...
void GuiControl::renderJustifiedText(Point2I offset, Point2I extent, const char *text, ColorI* fontOutlineColor) {
	...
	// Draw outline if neccessary
	if (mProfile->mOutlineFont) {
		offset.x += mProfile->mOutlineWidth * 2;

		ColorI oldMod(0, 0, 0, 255);
		dglGetBitmapModulation(&oldMod);
		
		ColorI newMod((fontOutlineColor != NULL ? *fontOutlineColor : mProfile->mOutlineColor));
		newMod.alpha = oldMod.alpha;
		dglSetBitmapModulation(newMod);

		// Top left
		Point2I pos((start + offset) - Point2I(mProfile->mOutlineWidth, mProfile->mOutlineWidth));
		dglDrawText(font, pos, text);

		// Top right
		pos.x += mProfile->mOutlineWidth * 2;
		dglDrawText(font, pos, text);

		// Bottom left
		pos.x -= mProfile->mOutlineWidth * 2;
		pos.y += mProfile->mOutlineWidth * 2;
		dglDrawText(font, pos, text);
