	...
	for (S32 i = 0; i < primitives.size(); i++)
	{
		...
		if (doColorShift) {
			if (!(draw.matIndex & TSDrawPrimitive::NoMaterial)) {
				...
				glColor4f(colorShift.red, colorShift.green, colorShift.blue, colorShift.alpha);
				glEnable(GL_TEXTURE_2D);

				if (colorShift.alpha < 0.9) {
					glEnable(GL_BLEND);
			...
			else if (colorShift.alpha >= 0.9) {
				glEnable(GL_TEXTURE_2D);
				glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
			}
			else {
				glEnable(GL_COLOR_MATERIAL);
				glColor4f(1.f, 1.f, 1.f, colorShift.alpha);
				glEnable(GL_TEXTURE_2D);
				glEnable(GL_BLEND);
				glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
				glDisable(GL_DEPTH);
