...
void TSShapeInstance::buildInstanceData(TSShape * _shape, bool loadMaterials)
{
   ...
   for (S32 i = 0; i < _shape->objects.size(); i++) nodeColors.push_back(ColorI(0, 0, 0, 255));

   nodeVisibility.setSize(_shape->objects.size());
   nodeVisibility.set();
}
...
ColorI TSShapeInstance::getNodeColor(const char* name) {
	S32 idx = hShape->findObject(name);
	if (idx == -1) {
		Con::errorf("TSShapeInstance::getNodeColor() - Failed to find node \"%s\"", name);
...
bool TSShapeInstance::setNodeColor(const char* name, ColorI color) {
	S32 idx = hShape->findObject(name);
	if (idx == -1) {
		if (!dStricmp(name, "ALL")) {
			for (idx = 0; idx < hShape->objects.size(); idx++) nodeColors[idx] = color;
			updateNodeColors();
			return true;
		}

		Con::errorf("TSShapeInstance::setNodeColor() - Failed to find node \"%s\"", name);
		return false;
	}

	nodeColors[idx] = color;
	updateNodeColors();
	return true;
}

bool TSShapeInstance::setNodeColor(S32 index, ColorI color) {
	if (index < 0 || index >= nodeColors.size()) return false;
	nodeColors[index] = color;
	return true;
}

void TSShapeInstance::updateNodeColors() {
	for (S32 i = 0; i < hShape->objects.size(); i++) {
		TSShapeInstance::MeshObjectInstance* inst = &mMeshObjects[i];

		for (S32 meshIdx = 0; meshIdx < inst->object->numMeshes; meshIdx++) {
			TSMesh* mesh = inst->getMesh(meshIdx);
			if (mesh == NULL) continue;

			mesh->doColorShift = (nodeColors[i] == ColorI(0, 0, 0, 255) ? false : true);
			mesh->colorShift   = ColorF((F32)nodeColors[i].red / 255.0f, (F32)nodeColors[i].green / 255.0f, (F32)nodeColors[i].blue / 255.0f, (F32)nodeColors[i].alpha / 255.0f);
		}
	}
}

bool TSShapeInstance::setNodeVisible(const char* name, bool val) {
	S32 idx = hShape->findObject(name);
	if (idx == -1) {
		if (!dStricmp(name, "ALL")) {
			for (idx = 0; idx < hShape->objects.size(); idx++) {
				if (val) nodeVisibility.set(idx);
				else nodeVisibility.clear(idx);
			}
			return true;
		}

		Con::errorf("TSShapeInstance::setNodeVisible() - Failed to find node \"%s\"", name);
		return false;
	}
...
bool TSShapeInstance::isNodeVisible(const char* name) {
	S32 index = hShape->findObject(name);
	if (index == -1) {
		Con::errorf("TSShapeInstance::isNodeVisible() - Failed to find node \"%s\"", name);
		return false;
	}

	return nodeVisibility.test(index);
}

bool TSShapeInstance::isNodeVisible(S32 index) {
	return nodeVisibility.test(index);
}
...
void TSShapeInstance::render(S32 dl, F32 intraDL, const Point3F * objectScale) {
	...
	if (!supportBuffers || !renderMeshesX(ss, od))
	{
		...
		updateNodeColors();
		for (i = start; i < end; i++) {
			if (!nodeVisibility.test(i)) continue;
...
bool TSShapeInstance::renderMeshesX(S32 ss, S32 od)
{
   ...
   if ((vb = mShape->mVertexBuffer) == -1)
   {
      ...
      // run through the meshes -- fill vertex buffer
      glLockVertexBufferEXT(vb,0);
	  for (i = start; i < end; ++i) mMeshObjects[i].fillVB(vb, mMaterialList);
      glUnlockVertexBufferEXT(vb);
   }
   ...
   if (mShape->mMorphable)
   {
      ...
      glLockVertexBufferEXT(vb,0);
	  for (i = start; i < end; ++i) mMeshObjects[i].morphVB(vb, mShape->mPreviousMerge[i], od, mMaterialList);
      glUnlockVertexBufferEXT(vb);
      PROFILE_END();
   }
   ...
   for (i = start; i < end; ++i) mMeshObjects[i].renderVB(vb, od, mMaterialList);
   PROFILE_END();
