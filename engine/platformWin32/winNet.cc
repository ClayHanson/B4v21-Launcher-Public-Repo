...
NetSocket Net::openPrivateConnectionTo(const char* addressString)
{
	if (!dStrnicmp(addressString, "ipx:", 4))
		return InvalidSocket;
	if (!dStrnicmp(addressString, "ip:", 3))
...
void Net::process()
{
  ...
  processSSL();
   ...
   for(;;)
   {
	  ...
	  //Con::printf("Received %d byte%s", bytesRead, bytesRead == 1 ? "" : "s");
