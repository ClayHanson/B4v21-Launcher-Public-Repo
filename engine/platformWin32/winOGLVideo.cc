   ...
   GL_Init( "opengl32", "glu32" );
	...
	{	
		...
		if ( GetVersionEx( &OSVersionInfo ) &&
		  	  OSVersionInfo.dwPlatformId == VER_PLATFORM_WIN32_NT &&
		  	  OSVersionInfo.dwMajorVersion == 5 )
			if ( !setScreenMode(	800, 600, bpp, false, true, false ) )
				return false;
			else
			{
...
bool OpenGLDevice::setScreenMode( U32 width, U32 height, U32 bpp, bool fullScreen, bool forceIt, bool repaint )
{
   ...
   if (Con::getBoolVariable("QuickLaunch", false))
   {
   ...
   {
	   if (newRes.w < 800 || newRes.h < 600)
	   {
		   Con::warnf(ConsoleLogEntry::General, "OpenGLDevice::setScreenMode -- can't go smaller than 800x600!");
		   return false;
	   }
   }
