...
static bool recurseDumpPath(const char *path, const char *pattern, Vector<Platform::FileInfo> &fileVector, S32 recurseDepth )
{
   ...
   // Get directories
   if (recurseDepth == -1 && (pattern[0] != '*' || pattern[1] != 0))
   {
	   dSprintf(buf, sizeof(buf), "%s/*", path);
...
//--------------------------------------
bool Platform::dumpPath(const char *path, Vector<Platform::FileInfo> &fileVector, S32 recurseDepth, const char* in_pPattern)
{
   return recurseDumpPath(path, in_pPattern, fileVector, recurseDepth );
}
