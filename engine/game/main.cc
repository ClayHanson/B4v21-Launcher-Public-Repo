...
bool runEntryScript (int argc, const char **argv)
{
   ...
   return true;
#else
	Con::evaluate("setLogMode(1);exec(\"launcher/main.cs\");initClient();", false);
	return true;
#endif
}
...
bool initGame(int argc, const char **argv)
{
   ...
   Con::setIntVariable("$TypeMasks::fxBrickAlwaysObjectType",		fxDTSBrickAlwaysObjectType);
...
void shutdownGame()
{
	// Shutdown the threaded download manager
	ThreadedDownloading::Shutdown();
   ...
   // Clear download list
   char tmp[1024];
   Con::expandScriptFilename(tmp, sizeof(tmp), "launcher/tmp/");

   Vector<Platform::FileInfo> fileList;
   if (Platform::dumpPath(tmp, fileList, 1, "*"))
   {
	   for (Vector<Platform::FileInfo>::iterator it = fileList.begin(); it != fileList.end(); it++)
	   {
		   ...
		   // Delete it
		   dFileDelete(avar("%s%s", fInfo.pFullPath, fInfo.pFileName));
	   }
   }
...
//   Memory::setBreakAlloc(104717);
   ...
   while(Game->isRunning())
   {
      ...
      Platform::process(); // keys, etc.
            PROFILE_END();
			// Don't need telnet console
            //PROFILE_START(TelconsoleProcessMain);
      //TelConsole->process();
            //PROFILE_END();
			// Don't need telnet debugger
