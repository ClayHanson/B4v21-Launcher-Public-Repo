...
class TSStatic : public SceneObject
{
   ...
   enum MaskBits {
	  ...
	  NodeVisibleMask = Parent::NextFreeMask << 1,
	  NodeColorMask = Parent::NextFreeMask << 2,
...
public:
	BitVector cache_nodeVisibility;
	Vector<ColorI> cache_nodeColors;

	void setNodeColor(const char* name, ColorI color);
	void setNodeVisible(const char* name, bool val);
