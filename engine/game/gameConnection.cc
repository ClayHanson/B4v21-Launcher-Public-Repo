...
const U32 GameConnection::CurrentProtocolVersion = 21;
const U32 GameConnection::MinRequiredProtocolVersion = 9;
...
void GameConnection::setPlayerName(const char* name) {
	if (*clientName != 0 || *name != 0) return;
	clientName = StringTable->insert(name, true);
}

void GameConnection::setGameID(S32 gameID) {
	if (clientGameID != -1 || gameID < 0) return;
	gameID = clientGameID;
}

S32 GameConnection::getGameID() {
	return clientGameID;
}

StringTableEntry GameConnection::getPlayerName() {
	return clientName;
}
...
ConsoleMethod(GameConnection, setPlayerName, void, 3, 3, "")
{
	object->setPlayerName(argv[2]);
}

ConsoleMethod(GameConnection, setGameID, void, 3, 3, "")
{
	object->setGameID(dAtoi(argv[2]));
}

ConsoleMethod(GameConnection, getPlayerName, const char*, 2, 2, "")
{
	StringTableEntry name = object->getPlayerName();
...
ConsoleMethod(GameConnection, getGameID, S32, 2, 2, "")
{
