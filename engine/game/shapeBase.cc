...
void ShapeBase::setNodeColor(const char* name, ColorI color) {
	if (mShapeInstance->setNodeColor(name, color)) setMaskBits(NodeColorMask);
}

void ShapeBase::setNodeVisible(const char* name, bool val) {
	if (mShapeInstance->setNodeVisible(name, val)) setMaskBits(NodeVisibleMask);
}
...
U32 ShapeBase::packUpdate(NetConnection *con, U32 mask, BitStream *stream)
{
   ...
   if(!stream->writeFlag(mask & (NodeVisibleMask | NameMask | DamageMask | SoundMask |
   ...
   if (stream->writeFlag(mask & NodeVisibleMask)) {
	   for (int i = 0; i < mDataBlock->shape->objects.size(); i++) stream->writeFlag(mShapeInstance->nodeVisibility.test(i));
   }

   // Node coloring
   if (stream->writeFlag(mask & NodeColorMask)) {
	   for (int i = 0; i < mDataBlock->shape->objects.size(); i++) {
...
ConsoleMethod(ShapeBase, setNodeVisible, void, 4, 4, "( string name, bool visible )")
{
...
ConsoleMethod(ShapeBase, hideNode, void, 3, 3, "( string name ) - Backwards compatibility")
{
	object->setNodeVisible(argv[2], 0);
}

ConsoleMethod(ShapeBase, unHideNode, void, 3, 3, "( string name ) - Backwards compatibility")
{
	object->setNodeVisible(argv[2], 1);
}

ConsoleMethod( ShapeBase, isNodeVisible, bool, 3, 3, "( string name )")
{
	return object->getShapeInstance()->isNodeVisible(argv[2]);
}

ConsoleMethod(ShapeBase, setNodeColor, void, 4, 4, "( string name, color )")
{
	float r = 0.0;
	float g = 0.0;
	float b = 0.0;
...
ConsoleMethod(ShapeBase, getNodeColor, const char*, 3, 3, "( string name )")
{
	if (!object->getShapeInstance()) return StringTable->insert("0 0 0 0");
	ColorI col = object->getShapeInstance()->getNodeColor(argv[2]);
...
ConsoleMethod(ShapeBase, setMeshVert, void, 7, 7, "( mesh_index, vertex_index, x, y, z )")
{
	// Arguments
	S32 mesh_index = dAtoi(argv[2]);
	S32 vert_index = dAtoi(argv[3]);
	S32 vert_xposi = dAtoi(argv[4]);
	S32 vert_yposi = dAtoi(argv[5]);
	S32 vert_zposi = dAtoi(argv[6]);
	...
	if (mesh_index >= shape->meshes.size()) {
		Con::errorf("%s() - mesh_index out of range; index = %d, size = %d", mesh_index, shape->meshes.size());
		return;
	}

	if (vert_index >= shape->meshes[mesh_index]->verts.size()) {
		Con::errorf("%s() - vert_index out of range; index = %d, size = %d", vert_index, shape->meshes[mesh_index]->verts.size());
		return;
	}

	shape->meshes[mesh_index]->verts[vert_index].x = vert_xposi;
	shape->meshes[mesh_index]->verts[vert_index].y = vert_yposi;
	shape->meshes[mesh_index]->verts[vert_index].z = vert_zposi;
}
