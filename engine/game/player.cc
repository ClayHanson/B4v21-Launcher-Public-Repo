...
PlayerData::ActionAnimationDef PlayerData::ActionAnimationList[NumTableActionAnims] =
{
   ...
   // Crouching
   { "crouch",  { 0.0f, 1.0f, 0.0f } },       // RunForwardAnim,
   { "crouchrun", { 0.0f, 1.0f, 0.0f } },       // BackBackwardAnim
   { "crouchback", { 0.0f, -1.0f, 0.0f } },       // BackBackwardAnim
   { "crouchside", { -1.0f, 0.0f, 0.0f } },       // SideLeftAnim,
...
PlayerData::PlayerData()
{
   ...
   crouchBoxSize.set(1.0f, 1.0f, 1.0f);
...
void Player::getDamageLocation(const Point3F& in_rPos, const char *&out_rpVert, const char *&out_rpQuad)
{
   ...
   F32 zHeight = mCrouching ? mDataBlock->crouchBoxSize.z : mDataBlock->boxSize.z;
   ...
   {
      F32 backToFront = mCrouching ? mDataBlock->crouchBoxSize.x : mDataBlock->boxSize.x;
      F32 leftToRight = mCrouching ? mDataBlock->crouchBoxSize.y : mDataBlock->boxSize.y;
...
void Player::updateMove(const Move* move)
{
   ...
   // Crouching
   setCrouching(move->trigger[3]);

   // Jetting
   if (mDataBlock->canJet) setJetting(move->trigger[4]);
   ...
   if (runSurface || mDataBlock->airControl != 0.0) {
      mContactTimer = 0;
      ...
      F32 maxAcc = ((mDataBlock->runForce * (runSurface ? mObjScale.z : -mDataBlock->airControl)) / mMass) * TickSec;
      if (mState == RecoverState)
   ...
   if (mJetting) {
	   if (!mCrouching) {
		   mVelocity.z += (70.0 / mMass);
	   } else {
		   const MatrixF& mat = getTransform();
		   Point3F dir; mat.getColumn(1, &dir);

		   mVelocity += dir * (70.0 / mMass);
	   }
   }
...
void Player::pickActionAnimation()
{
   ...
   bool forward = true;
   U32 action = (mCrouching ? PlayerData::CrouchIdleAnim : PlayerData::RootAnim);
   if (mFalling && !mCrouching)
   {
   ...
   else
   {
      if (mContactTimer >= sContactTickTime) {
         ...
         action = (mCrouching ? PlayerData::CrouchIdleAnim : PlayerData::RootAnim);
      }
      else
      {
         ...
         VectorF vel;
         mWorldToObj.mulV(mVelocity,&vel);
         for (U32 i = (mCrouching ? PlayerData::CrouchForwardAnim : 1); i < (mCrouching ? PlayerData::CrouchIdleAnim : 0) + PlayerData::NumMoveActionAnims; i++)
         {
            PlayerData::ActionAnimation &anim = mDataBlock->actionList[i];
            if (anim.sequence != -1 && anim.speed) {
               ...
               else
               {
                  ...
                  if (i == (mCrouching ? PlayerData::CrouchSideAnim : PlayerData::SideLeftAnim) && -d > curMax)
                  {
                     curMax = -d;
                     action = i;
                     forward = false;
   ...
   if (mJetting) action = (mCrouching ? PlayerData::CrouchIdleAnim : PlayerData::RootAnim);
...
void Player::setJetting(bool val) {
	if (mJetting == val) return;

	mJetting = val;
	if(isServerObject()) setMaskBits(ActionMask);
}

void Player::setCrouching(bool val) {
	if (mCrouching == val) return;

	mCrouching = val;
	if(val) setThreadSequence(0, getShape()->findSequence("crouch"), true);
	else setThreadSequence(0, getShape()->findSequence("root"), true);

	mObjBox.max.x = (mCrouching ? mDataBlock->crouchBoxSize.x : mDataBlock->boxSize.x) * 0.5f;
	mObjBox.max.y = (mCrouching ? mDataBlock->crouchBoxSize.y : mDataBlock->boxSize.y) * 0.5f;
	mObjBox.max.z = (mCrouching ? mDataBlock->crouchBoxSize.z : mDataBlock->boxSize.z);
	mObjBox.min.x = -mObjBox.max.x;
	mObjBox.min.y = -mObjBox.max.y;
	mObjBox.min.z = 0.0f;
...
U32 Player::packUpdate(NetConnection *con, U32 mask, BitStream *stream)
{
   ...
   if (stream->writeFlag(mask & FaceNameMask)) stream->writeString(mFaceName, 32);
   if (stream->writeFlag(mask & DecalNameMask)) stream->writeString(mDecalName, 32);
...
void Player::unpackUpdate(NetConnection *con, BitStream *stream)
{
   ...
   if (stream->readFlag()) { char faceName[33]; stream->readString(faceName); setFaceName(faceName); }
   if (stream->readFlag()) { char decalName[33]; stream->readString(decalName); setDecalName(decalName); }
...
void Player::renderImage(SceneState* state, SceneRenderImage* image)
{
   ...
   if (mShapeInstance && renderPlayer && DetailManager::selectCurrentDetail(mShapeInstance)) {
	  ...
	  if(mFaceIdx != -1) {
		  ...
		  S32 idx        = shape->findIflMaterial("face.ifl");
		  TextureHandle &handle = shape->materialList->mMaterials[shape->iflMaterials[idx].materialSlot];
		  shape->materialList->load(0, shape->materialList->getMaterialName(mFaceIdx));
	  }
...
void Player::setFaceName(const char* name) {
	if (!mShapeInstance) return;

	TSShape* shape = mShapeInstance->getShape();
	if (isClientObject()) if (!mShapeInstance->ownMaterialList()) mShapeInstance->cloneMaterialList();

	S32 idx = shape->findIflMaterial("face.ifl");
	if (idx == -1) return;

	S32 start = shape->iflMaterials[idx].firstFrame;
	S32 end   = start + shape->iflMaterials[idx].numFrames;

	for (S32 i = start; i <= end; i++) {
		if (i == end) {
			if (!dStricmp(name, "smiley")) {
				Con::errorf("Failed to apply ANY face (couldn't even find smiley.png)");
				return;
			}

			Con::warnf("Failed to find face \"%s\"", name);
			name = "smiley";
			i    = start - 1;
			continue;
		}

		const char* matName = shape->materialList->getMaterialName(i);
		if (matName == NULL) continue;
		{ const char* ptr = 0; while ((ptr = dStrstr(matName, "/")) != NULL) matName = ptr + 1; }

		if (!dStricmp(matName, name)) {
			mFaceName = StringTable->insert(name);
			mFaceIdx  = i;

			if (isClientObject()) mShapeInstance->mIflMaterialInstances[idx].frame = i - start;
			if (isServerObject()) setMaskBits(FaceNameMask);
			return;
		}
	}
}

void Player::setDecalName(const char* name) {
	if (!mShapeInstance) return;

	TSShape* shape = mShapeInstance->getShape();
	if (isClientObject()) if (!mShapeInstance->ownMaterialList()) mShapeInstance->cloneMaterialList();

	S32 idx = shape->findIflMaterial("decal.ifl");
	if (idx == -1) return;
	...
	for (S32 i = start; i <= end; i++) {
		if (i == end) {
			if (!dStricmp(name, "AAA-None")) {
				Con::errorf("Failed to apply ANY face (couldn't even find smiley.png)");
				return;
			}

			Con::warnf("Failed to find decal \"%s\"", name);
			name = "AAA-None";
			i = start - 1;
			continue;
		}
		...
		if (!dStricmp(matName, name)) {
			mDecalName = StringTable->insert(name);
			mDecalIdx  = i;

			if (isClientObject()) mShapeInstance->mIflMaterialInstances[idx].frame = i - start;
			if (isServerObject()) setMaskBits(DecalNameMask);
			return;
		}
	}
}

ConsoleMethod(Player, setFaceName, void, 3, 3, "(faceName)") {
	object->setFaceName(argv[2]);
}

ConsoleMethod(Player, setDecalName, void, 3, 3, "(faceName)") {
