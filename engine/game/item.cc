...
ItemData::ItemData()
{
   ...
   doColorShift    = false;
   colorShiftColor = ColorF(0.0, 0.0, 0.0, 1.0);
...
void ItemData::packData(BitStream* stream)
{
   ...
   if (stream->writeFlag(doColorShift))
...
void ItemData::unpackData(BitStream* stream)
{
   ...
   if ((doColorShift = stream->readFlag()))
                           ...
                           const bool  modifyBaseState)
{
   ...
   if (mDataBlock->doColorShift) {
	   ColorI cShift(mDataBlock->colorShiftColor.red * 255.0, mDataBlock->colorShiftColor.green * 255.0, mDataBlock->colorShiftColor.blue * 255.0, mDataBlock->colorShiftColor.alpha * 255.0);
	   if (mShapeInstance->nodeColors[0] != cShift) mShapeInstance->setNodeColor("ALL", cShift);
   }
...
/*void Item::renderShadow(F32 dist, F32 fogAmount)
{
	if(!mDataBlock->shadowEnable)
		return;

	// this doesn't work yet, it seems item shadows in TGE never worked...
	// just use base class shadow code...
	bool allowmove = mDataBlock->shadowCanMove && !mAtRest;
	bool allowanimate = mDataBlock->shadowCanAnimate && !mAtRest && !mRotate;
