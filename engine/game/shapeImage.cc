...
void ShapeBase::setImage(U32 imageSlot, ShapeBaseImageData* imageData, StringHandle& skinNameHandle, bool loaded, bool ammo, bool triggerDown, bool target)
{
   ...
   // Update the image's shapeinstance nodecolors
   if (imageData->doColorShift) {
	   ColorI cShift(imageData->colorShiftColor.red * 255.0, imageData->colorShiftColor.green * 255.0, imageData->colorShiftColor.blue * 255.0, imageData->colorShiftColor.alpha * 255.0);
	   if (image.shapeInstance->nodeColors[0] != cShift) image.shapeInstance->setNodeColor("ALL", cShift);
   }
