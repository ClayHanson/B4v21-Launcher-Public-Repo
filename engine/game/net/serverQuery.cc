...
#define MAX_SIMPLE_PING 128
NetAddress simplePingAddress[MAX_SIMPLE_PING];
U32 simplePingTimeoutEventId[MAX_SIMPLE_PING];
U32 simplePingTime[MAX_SIMPLE_PING];
   ...
   for ( i = 0; i < gPingList.size() && i < gMaxConcurrentPings; )
   {
      ...
      if ( p.time + gPingTimeout < time )
      {
         ...
         else
         {
			...
			sendPacket(14, &p.address, p.key, p.session, flags);
            i++;
         }
      }
      else
      ...
      for ( U32 i = 0; i < gQueryList.size() && i < gMaxConcurrentQueries; )
      {
         ...
         if ( p.time + gPingTimeout < time )
         {
            ...
            else
            {
			   ...
			   Con::executef(4, "onSimplePingReceived", addressString);
               ...
               sendPacket( 18, &p.address, p.key, p.session, flags );
               if ( !si->isQuerying() )
               {
...
static void handleGamePingResponse( const NetAddress* address, BitStream* stream, U32 key, U8 /*flags*/ )
{
   ...
   {
      Con::printf( "Protocol for server %s does not meet minimum protocol (they responded with %d, we expected %d).", addrString, temp32, GameConnection::MinRequiredProtocolVersion );
      gFinishedList.push_back( *address );
      gPingList.erase( index );
      if ( si )
      {
   ...
   {
      Con::printf( "You do not meet the minimum protocol for server %s (%d).", addrString, temp32 );
      gFinishedList.push_back( *address );
      gPingList.erase( index );
      if ( si )
      {
...
static void handleGameInfoResponse( const NetAddress* address, BitStream* stream, U32 /*key*/, U8 /*flags*/ )
{
   ...
   Con::executef(10, "onLanPingReceived", addrString, (si->isPassworded() == true ? "1" : "0"), (si->isDedicated() == true ? "1" : "0"), Con::getIntArg((S32)si->numPlayers), Con::getIntArg((S32)si->maxPlayers), si->name, si->missionName, Con::getIntArg((S32)si->version), Con::getIntArg((S32)si->ping));
...
static void handleSimplePingResponse(const NetAddress* address, BitStream* stream, U32 key, U8 /*flags*/)
{
	char addrExpected[256];
	char indexBuff[32];
	char pingBuff[32];
	char addrGot[256];

	if (key >= MAX_SIMPLE_PING)
	{
		Con::errorf("Warning: handleSimplePingResponse() - out of range key recieved %d", key);
		return;
	}

	// Get address strings
	Net::addressToString(&simplePingAddress[key], addrExpected);
	Net::addressToString(address, addrGot);

	// Do match
	int match = dStrcmp((const char*)addrExpected, (const char*)addrGot);
	if (match)
	{
		Con::warnf("Warning: handleSimplePingResponse() - address does not match");
		Con::warnf("  expected %s, got %s", addrExpected, addrGot);
		return;
	}

	if (!simplePingTime[key])
	{
		Con::warnf("handleSimplePingResponse() - ping response recieved after 1000ms - ignoring");
		return;
	}

	// Cancel the timeout event
	Sim::cancelEvent(simplePingTimeoutEventId[key]);

	// Get the ping
	if (Platform::getVirtualMilliseconds() > simplePingTime[key])
		match = Platform::getVirtualMilliseconds() - simplePingTime[key];

	// Ready buffers
	dSprintf(pingBuff, sizeof(pingBuff), "%d", match);
	dSprintf(indexBuff, sizeof(indexBuff), "%d", key);
	
	// Reset simple ping buffer
	simplePingTimeoutEventId[key] = 0;
	simplePingTime[key]           = 0;

	// Execute!
	Con::executef(4, "onSimplePingReceived", addrGot, pingBuff, indexBuff);
}
...
void DemoNetInterface::handleInfoPacket( const NetAddress* address, U8 packetType, BitStream* stream )
{
   ...
   switch( packetType )
   {
	  ...
	  case SimplePingResponse:
		  handleSimplePingResponse(address, stream, key, flags);
		  break;
   }
}

void onPingTimeout(void* uData)
{
	char addr[256];
	char idx[128];
	int index = (int)uData;
	...
	// Reset stuff
	simplePingTimeoutEventId[index] = 0;
	simplePingTime[index]           = 0;

	// Populate strings
	Net::addressToString(&simplePingAddress[index], addr);
	dSprintf(idx, sizeof(idx), "%d", index);

	// Execute it
	Con::executef(3, "onSimplePingTimeout", StringTable->insert(addr), StringTable->insert(idx));
}

ConsoleFunction(pingSingleServer, void, 3, 3, "pingSingleServer(address, index);")
{
	char* addrHost = dStrdup(argv[1]);
	S32 index      = dAtoi(argv[2]);

	if (index >= MAX_SIMPLE_PING)
	{
		Con::errorf("Error: pingSingleServer() - tempKey out of range");
		return;
	}

	if (!Net::stringToAddress(addrHost, &simplePingAddress[index]))
	{
		Con::errorf("Error: pingSingleServer() - invalid address");
		return;
	}

	// Build the argv
	SimEngineEvent* eve             = new SimEngineEvent(onPingTimeout, (void*)index);
	simplePingTimeoutEventId[index] = Sim::postEvent(Sim::getRootGroup(), eve, Sim::getCurrentTime() + 1100);
	sendPacket(22, &simplePingAddress[index], index, 0, 0);
	simplePingTime[index]           = Platform::getVirtualMilliseconds();
}

ConsoleFunction(getMaxPingableServers, S32, 1, 1, "")
{
	return MAX_SIMPLE_PING;
}
